...
//
// Rewrites the coefficients around a stroke footprint's time edges so they
// carry the analysis of the time-domain-exact edit. Editing coefficients with
// a hard time boundary orphans the cross-boundary tails of atoms on both
// sides: content just outside the footprint rings back into it (audible as a
// pre-tail before audio "comes back in"), and the footprint's own content
// rings out of it. The exact result — what a time-domain gate or crossfade at
// the boundary would produce — differs from the coefficient edit only within
// one atom-support of each edge, so it is computed there and spliced back:
//
//   1. Split the touched bands' coefficients into the footprint interior and
//      exterior, and synthesize each stem over a window around the edges.
//   2. The intended audio keeps interior content strictly inside the
//      footprint's time span and exterior content strictly outside, so the
//      correction signal is  -yOut inside the span and -yIn outside it.
//   3. Analyze the correction and add its coefficients onto the canvas within
//      the window (plus a spectral skirt: a truly sudden edge of band-limited
//      content leaks a little beyond the touched bands).
//
// The input buffer is not modified; the changed pixels are returned as a
// patch (per-band ranges plus values) so the caller can decide atomically
// whether to apply it. New magnitudes/phases keep each pixel's unwrapped
// phase texture by placing the new phase on the turn nearest the old value.

static inline float wrapPiFloat(float x)
{
    return std::remainderf(x, 2.0f * static_cast<float>(M_PI));
}

class ConditionBoundaryWorker : public Napi::AsyncWorker
{
public:
    ConditionBoundaryWorker(Napi::Env env,
                            const Napi::Float32Array &packedJs,
                            const Napi::Object &metaObj,
                            double sampleRate,
                            const Napi::Object &paramsJs,
                            const Napi::Object &footObj)
        : Napi::AsyncWorker(env), deferred(Napi::Promise::Deferred::New(env)), sampleRate(sampleRate)
    {
        packedRef = Napi::Reference<Napi::Float32Array>::New(packedJs, 1);
        packed = packedJs.Data();
        ...
        footStart = footObj.Get("startFrame").As<Napi::Number>().Int64Value();
        footEnd = footObj.Get("endFrame").As<Napi::Number>().Int64Value();
        footBandLo = footObj.Get("bandLo").As<Napi::Number>().Int32Value();
        footBandHi = footObj.Get("bandHi").As<Napi::Number>().Int32Value();
    }

    void Execute()
    {
        if (footBandLo > footBandHi)
            std::swap(footBandLo, footBandHi);
        footBandLo = std::max(0, footBandLo);
        footBandHi = std::min(numBands - 1, footBandHi);
        footStart = std::max<int64_t>(0, footStart);
        footEnd = std::min<int64_t>(numFrames, footEnd);
        if (footStart >= footEnd || footBandLo > footBandHi)
            return;
        ...
        int64_t support = 0;
        for (int b = footBandLo; b <= footBandHi; ++b)
            support = std::max(support, static_cast<int64_t>(std::ceil(analyzer.band_synthesis_support(b + bandBegin))));
        support = std::min(support, static_cast<int64_t>(sampleRate * 2.0));

        // Correction windows around each edge that lies inside the file — an
        // edge on the file boundary is not a transition and needs nothing.
        std::vector<std::pair<int64_t, int64_t>> windows;
        if (footStart > 0)
            windows.push_back({footStart - support, std::min(footStart + support, footEnd)});
        if (footEnd < numFrames)
            windows.push_back({std::max(footEnd - support, footStart), footEnd + support});
        if (windows.size() == 2 && windows[1].first <= windows[0].second)
        {
            windows[0].second = windows[1].second;
            windows.pop_back();
        }
        for (auto &w : windows)
        {
            w.first = std::max<int64_t>(0, w.first);
            w.second = std::min<int64_t>(numFrames, w.second);
        }

        const int skirt = bandsPerOctave / 3;
        const int spliceLo = std::max(0, footBandLo - skirt);
        const int spliceHi = std::min(numBands - 1, footBandHi + skirt);
        const int64_t rampSamples = static_cast<int64_t>(sampleRate * 0.01);

        for (const auto &win : windows)
        {
            const int64_t w0 = win.first, w1 = win.second;
            if (w1 <= w0)
                continue;
            const size_t winLen = static_cast<size_t>(w1 - w0);

            // Copy the affected pixel rows into the patch before touching them,
            // so the patch holds current values plus corrections for exactly
            // the returned ranges.
            std::vector<size_t> patchBase(numBands, 0);
            for (int bi = spliceLo; bi <= spliceHi; ++bi)
            {
                const int64_t k0 = w0 >> bandStepLog2s[bi];
                const int64_t k1 = std::min<int64_t>(bandLengths[bi] - 1, (w1 - 1) >> bandStepLog2s[bi]);
                if (k1 < k0)
                    continue;
                patchBase[bi] = patchPixels.size() / 4;
                rangeBands.push_back(static_cast<uint32_t>(bi));
                rangeStarts.push_back(static_cast<uint32_t>(k0));
                rangeCounts.push_back(static_cast<uint32_t>(k1 - k0 + 1));
                const size_t srcOff = (bandOffsets[bi] + static_cast<size_t>(k0)) * 4;
                const size_t count = static_cast<size_t>(k1 - k0 + 1) * 4;
                const size_t prev = patchPixels.size();
                patchPixels.resize(prev + count);
                if (srcOff + count <= packedLen)
                    std::copy(packed + srcOff, packed + srcOff + count, patchPixels.begin() + prev);
            }
            const int64_t fillStart = std::max<int64_t>(0, w0 - 2 * support);
            const int64_t fillEnd = std::min<int64_t>(numFrames, w1 + 2 * support);

            for (int ch = 0; ch < channels; ++ch)
            {
                auto fillStem = [&](bool wantInside, gaborator::coefs<float> &coefs)
                {
                    gaborator::fill(
                        [&](int b, int64_t t, std::complex<float> &coef)
                        {
                            coef = {0.0f, 0.0f};
                            int bi = b - bandBegin;
                            if (bi < footBandLo || bi > footBandHi)
                                return;
                            const bool inside = t >= footStart && t < footEnd;
                            if (inside != wantInside)
                                return;
                            const int64_t k = t >> bandStepLog2s[bi];
                            if (k < 0 || k >= static_cast<int64_t>(bandLengths[bi]))
                                return;
                            const size_t off = (bandOffsets[bi] + static_cast<size_t>(k)) * 4 + ch * 2;
                            if (off + 1 >= packedLen)
                                return;
                            const float mag = packed[off];
                            const float ph = packed[off + 1];
                            coef = {mag * std::cos(ph), mag * std::sin(ph)};
                        },
                        bandBegin, bandBegin + numBands, fillStart, fillEnd, coefs);
                };

                gaborator::coefs<float> inCoefs(analyzer), outCoefs(analyzer);
                fillStem(true, inCoefs);
                fillStem(false, outCoefs);

                std::vector<float> yIn(winLen), yOut(winLen);
                analyzer.synthesize(inCoefs, w0, w1, yIn.data());
                analyzer.synthesize(outCoefs, w0, w1, yOut.data());

                // Interior keeps only interior content, exterior only exterior
                // content; the correction removes each stem's cross-boundary
                // tail. Short ramps at the window ends are insurance against a
                // residual step at the splice bounds (the correction is already
                // near zero there by construction).
                std::vector<float> corr(winLen);
                for (size_t i = 0; i < winLen; ++i)
                {
                    const int64_t t = w0 + static_cast<int64_t>(i);
                    const bool inside = t >= footStart && t < footEnd;
                    corr[i] = inside ? -yOut[i] : -yIn[i];
                }
                if (w0 > 0)
                    for (int64_t i = 0; i < rampSamples && i < static_cast<int64_t>(winLen); ++i)
                        corr[i] *= static_cast<float>(i) / static_cast<float>(rampSamples);
                if (w1 < numFrames)
                    for (int64_t i = 0; i < rampSamples && i < static_cast<int64_t>(winLen); ++i)
                        corr[winLen - 1 - i] *= static_cast<float>(i) / static_cast<float>(rampSamples);

                gaborator::coefs<float> corrCoefs(analyzer);
                analyzer.analyze(corr.data(), w0, w1, corrCoefs);

                gaborator::process(
                    [&](int b, int64_t t, std::complex<float> &c)
                    {
                        int bi = b - bandBegin;
                        if (bi < spliceLo || bi > spliceHi)
                            return;
                        const int64_t k = t >> bandStepLog2s[bi];
                        const int64_t k0 = w0 >> bandStepLog2s[bi];
                        const int64_t k1 = std::min<int64_t>(bandLengths[bi] - 1, (w1 - 1) >> bandStepLog2s[bi]);
                        if (k < k0 || k > k1)
                            return;
                        const size_t px = (patchBase[bi] + static_cast<size_t>(k - k0)) * 4 + ch * 2;
                        if (px + 1 >= patchPixels.size())
                            return;
                        const float mag = patchPixels[px];
                        const float ph = patchPixels[px + 1];
                        const std::complex<float> cur(mag * std::cos(ph), mag * std::sin(ph));
                        const std::complex<float> next = cur + c;
                        const float newMag = std::abs(next);
                        patchPixels[px] = newMag;
                        if (newMag > 1e-12f)
                        {
                            const float newPh = std::arg(next);
                            patchPixels[px + 1] = ph + wrapPiFloat(newPh - ph);
                        }
                    },
                    bandBegin + spliceLo, bandBegin + spliceHi + 1, w0, w1, corrCoefs);
            }
        }
    }
...
private:
    ...
    int64_t footStart = 0, footEnd = 0;
    int footBandLo = 0, footBandHi = 0;

    std::vector<uint32_t> rangeBands, rangeStarts, rangeCounts;
    std::vector<float> patchPixels;
};

Napi::Value ConditionBoundaryAsync(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    if (info.Length() < 5 || !info[0].IsTypedArray() || !info[1].IsObject() || !info[2].IsNumber() || !info[3].IsObject() || !info[4].IsObject())
    {
        Napi::TypeError::New(env, "Expected: packedData (Float32Array), meta (Object), sampleRate (Number), params (Object), footprint (Object)").ThrowAsJavaScriptException();
        return env.Null();
    }
    auto *worker = new ConditionBoundaryWorker(env, info[0].As<Napi::Float32Array>(), info[1].As<Napi::Object>(),
                                               info[2].As<Napi::Number>().DoubleValue(), info[3].As<Napi::Object>(),
