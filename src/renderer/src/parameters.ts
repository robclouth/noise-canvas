...
const baseParameterDefs: Partial<Record<ParameterKey, ParameterDefInput>> = {
  ...
  transformSpeed: {
    ...
    name: "Speed",
    label: "Speed",
    description: "Plays the sound faster or slower, moving its pitch and its length together.",
    default: 1.0,
    min: 1 / 16,
    max: 16,
    step: 0.001,
    scale: "log",
    unit: MULTIPLIER_UNIT,
    marks: MULTIPLIER_VALUES.filter((mark) => mark.value >= 1 / 16 && mark.value <= 16),
    includeInStep: true,
    modulatable: true,
    effectType: "transform",
  },
